...
def validate_client_and_vehicle(client_id: int, vehicle_id: int, workshop_id: int, db: Session):
    client = (
        db.query(Client)
        .filter(Client.id == client_id, Client.workshop_id == workshop_id)
        .first()
    )
    if not client:
        raise HTTPException(status_code=404, detail="Cliente no encontrado para este taller")
...
def create_work_order(
    ...
    validate_client_and_vehicle(work_order.client_id, work_order.vehicle_id, workshop_id, db)
...
def generate_invoice_pdf(
    ...
    client = db.query(Client).filter(Client.id == work_order.client_id, Client.workshop_id == current_user.workshop_id).first()
    vehicle = db.query(Vehicle).filter(Vehicle.id == work_order.vehicle_id, Vehicle.workshop_id == current_user.workshop_id).first()
    ...
    client_data = [
        ...
        ["Vehículo", f"{safe(getattr(vehicle, 'brand', None), '')} {safe(getattr(vehicle, 'model', None), '')}".strip() or "-", "Placa", safe(getattr(vehicle, "plate", None))],
    ...
    for item in items:
        ...
        item_rows.append([item_type, safe(item.description), f"{Decimal(item.quantity or 0):.2f}", money(item.unit_price), money(item.subtotal)])
    ...
    for title, value in [
        ("Problema reportado", work_order.issue_description),
        ("Diagnóstico", work_order.diagnosis),
        ("Trabajo realizado", work_order.work_performed),
        ("Notas", work_order.notes),
    ]:
        if value:
            story.append(Paragraph(title, heading))
            story.append(Paragraph(safe(value), normal))
            story.append(Spacer(1, 0.25 * cm))

    story.append(Spacer(1, 1 * cm))
    signatures = Table(
        [["____________________________", "____________________________"], ["Firma taller", "Firma cliente"]],
        colWidths=[8 * cm, 8 * cm],
    )
