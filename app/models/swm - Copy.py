...
class SwmVehicle(Base):
    ...
    user_id = Column(Integer, ForeignKey("swm_users.id", ondelete="CASCADE"), nullable=True, index=True)
    ...
    service_records = relationship("SwmServiceRecord", back_populates="vehicle", cascade="all, delete-orphan")
    service_orders = relationship("SwmServiceOrder", back_populates="vehicle", cascade="all, delete-orphan")
    ai_queries = relationship("SwmAiQuery", back_populates="vehicle", cascade="all, delete-orphan")
...
class SwmServiceOrder(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    vehicle_id = Column(Integer, ForeignKey("swm_vehicles.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    records = relationship("SwmServiceRecord", back_populates="service_order", cascade="all, delete-orphan")
...
class SwmServiceRecord(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    vehicle_id = Column(Integer, ForeignKey("swm_vehicles.id", ondelete="CASCADE"), nullable=False, index=True)
    service_order_id = Column(Integer, ForeignKey("swm_service_orders.id", ondelete="CASCADE"), nullable=True, index=True)
    schedule_id = Column(Integer, ForeignKey("swm_maintenance_schedule.id"), nullable=True)
    service_mileage = Column(Integer, nullable=False)
    service_date = Column(Date, nullable=False)
    workshop = Column(String(150), nullable=True)
...
class SwmAiQuery(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    vehicle_id = Column(Integer, ForeignKey("swm_vehicles.id", ondelete="SET NULL"), nullable=True, index=True)
